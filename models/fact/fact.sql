...
WITH FACT AS (
  SELECT    *,
            ...
            TO_CHAR(LAST_LOGGED_IN, 'YYYYMMDD')::NUMBER(8) AS LAST_LOGGED_IN_KEY,
  ...
  FROM {{ref("combined_data")}}
                      ...
                      NVL(DATE_COMPLETED_KEY, -999) AS DATE_COMPLETED_KEY,
                      NVL(DATE_EDITED_KEY, -999) AS DATE_EDITED_KEY,
                      NVL(DATE_ENROLLED_KEY, -999) AS DATE_ENROLLED_KEY,
                      NVL(DATE_EXPIRES_KEY, -999) AS DATE_EXPIRES_KEY,
                      NVL(LAST_LOGGED_IN_KEY, -999) AS LAST_LOGGED_IN_KEY,
